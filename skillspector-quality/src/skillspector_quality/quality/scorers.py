...
def dim_behavioral_config(doc: SkillDoc, w: int) -> list[tuple[int, int, str]]:
    ...
    if "context" in data:
        ...
        _ctx_mark = "✓" if valid_ctx else "— invalid (only 'fork' is allowed)"
        exec_parts.append(f"context: {ctx} {_ctx_mark}")
    ...
    if "effort" in data:
        ...
        _effort_mark = "✓" if valid else ("— invalid (must be one of: " + ", ".join(sorted(_VALID_EFFORT)) + ")")
        exec_parts.append(
            f"effort: {val} {_effort_mark}"
        )
