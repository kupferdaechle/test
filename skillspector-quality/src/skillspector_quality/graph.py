...
def build_graph() -> CompiledStateGraph[Any, Any, Any]:
    ...
    # When no analyzers are registered (stub mode), wire build_context directly to
    # meta_analyzer so the report node is always reachable.
    if not ANALYZER_NODE_IDS:
        workflow.add_edge("build_context", "meta_analyzer")
